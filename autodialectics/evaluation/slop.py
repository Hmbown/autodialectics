...
class SlopScorer:
    ...
    def score(
        ...
        *,
        ...
        for metric_name, weight in self._WEIGHTS.items():
            ...
            total_weight += weight
        metrics.composite = composite / total_weight if total_weight > 0 else 0.0
    ...
    def _unsupported_claims(
        ...
        return min(unsupported / max(claim_count, 1) * (1.0 - uncertainty_ratio), 1.0)
...
class RunEvaluator:
    ...
    def verify(
        ...
        *,
        ...
        for criterion in contract.acceptance_criteria:
            # Check if criterion keywords appear in execution output
            criterion_kw = keyword_set(criterion)
            text_kw = keyword_set(text)
            overlap = len(criterion_kw & text_kw) / max(len(criterion_kw), 1)
            ...
            checks.append(
                VerificationCheck(
                    criterion=criterion,
                    status=status,
                    notes=f"Keyword overlap: {overlap:.2f}",
                )
            )
    ...
    def evaluate_run(
        ...
        unsupported_assertion_rate = slop.unsupported_claims

        # Redundancy rate
        redundancy_rate = slop.repetition_without_progress

        # Novelty usefulness (proxy: inverse of shallow_novelty + benchmark_gaming)
        novelty_usefulness = 1.0 - (
            slop.shallow_novelty * 0.5 + slop.benchmark_gaming * 0.5
        )

        # Requirement fidelity
        requirement_fidelity = 1.0 - slop.requirement_drift

        # Verification quality
        verification_quality = verification.confidence
